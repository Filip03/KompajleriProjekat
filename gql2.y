...
void print_ast(Node *n, int indent) {
    if (!n) return; // Handle NULL node gracefully
    int i = 0;
...
// External declarations for yylex and yyparse
...
%token T_PLUS T_PLUSPLUS T_MINUS T_MINUSMINUS T_STAR T_STARS T_PIPE
%token T_LT T_GT T_LBRACK T_RBRACK T_LPAREN T_RPAREN T_EQ T_COLON T_SEMI T_COMMA

// Define precedence and associativity (highest to lowest)
%right T_PLUS T_MINUS T_STAR // Unary operators
%left T_PIPE                // Logical OR (Alternative operator)

// New type declarations for the restructured grammar
...
commands: command
| commands command {
    $$ = create_node(NODE_PROGRAM, NULL, 2, $1, $2); // Assuming program node can be used to group commands
}
;
...
| identifier T_EQ identifier T_PLUSPLUS identifier {
    $$ = create_node(NODE_SET_OP, "++", 2,
        create_node(NODE_TERM, $3, 0),
        create_node(NODE_TERM, $5, 0));
}
| identifier T_EQ identifier T_MINUSMINUS identifier {
    $$ = create_node(NODE_SET_OP, "--", 2,
        create_node(NODE_TERM, $3, 0),
        create_node(NODE_TERM, $5, 0));
}
| identifier T_EQ identifier T_STARS identifier {
    $$ = create_node(NODE_SET_OP, "**", 2,
        create_node(NODE_TERM, $3, 0),
        create_node(NODE_TERM, $5, 0));
}
...
| identifier {
    $$ = create_node(NODE_QUERY, $1, 0); // Representing a reference to a declared query
}
;
...
// --- New Precedence Handling ---

// Lowest precedence: PIPE operator (logical OR)
...
// Next precedence: Juxtaposition (implicit space between terms)
...
unary_expression: primary_expression
                | operator unary_expression { // Unary operators: +term, -term, *term
                    $$ = create_node($1->type, NULL, 1, $2);
                    free($1); // Free the temporary operator node
                }
;

// Base elements (words, strings, directives)
primary_expression: T_WORD {
                  ...
                  // Optional: Parentheses for custom grouping if needed, add 'T_LPAREN' 'T_RPAREN' tokens if not already present.
                  // | T_LPAREN query_expression T_RPAREN { $$ = $2; }
;


// --- End New Precedence Handling ---
...
int main() {
    // By default, yyin reads from stdin.
    // To test with a specific file (e.g., op-precedence-input.txt), you can uncomment and modify:
    // FILE *f = fopen("op-precedence-input.txt", "r");
    // if (f) {
    //     yyin = f;
    // } else {
